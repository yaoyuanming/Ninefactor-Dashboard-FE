define([
  ...
  Je.computeVertices = function (e) {
    var t,
      ...
      l = Math.atan,
      o = Math.exp,
      f = We.CesiumMath.PI_OVER_TWO,
      u = We.CesiumMath.toRadians,
      c = e.heightmap,
      d = e.width,
      h = e.height,
      m = e.skirtHeight,
      g = Ye.defaultValue(e.isGeographic, !0),
      p = Ye.defaultValue(e.ellipsoid, Xe.Ellipsoid.WGS84),
      w = 1 / p.maximumRadius,
      x = e.nativeRectangle,
      k = e.rectangle,
      y = Ye.defined(k)
      ...
      U = Ye.defaultValue(e.exaggeration, 1),
      v = Ye.defaultValue(e.includeWebMercatorT, !1),
      ...
      P = Xe.Rectangle.computeWidth(x),
      E = Xe.Rectangle.computeHeight(x),
      C = P / (d - 1),
      F = E / (h - 1);
    g || ((P *= w), (E *= w));
    var L,
      ...
      H = 65536,
      Y = -65536,
      W = Ze.Transforms.eastNorthUpToFixedFrame(I, p),
      X = Ze.Matrix4.inverseTransformation(W, $e);
    v &&
      ((L = je.WebMercatorProjection.geodeticLatitudeToMercatorAngle(a)),
      (O =
        1 / (je.WebMercatorProjection.geodeticLatitudeToMercatorAngle(y) - L)));
    var Z = et;
    ...
    for (var ne = te; ne < ae; ++ne) {
      var se = ne;
      se < 0 && (se = 0), h <= se && (se = h - 1);
      var le = x.north - F * se,
        oe = ((le = g ? u(le) : f - 2 * l(o(-le * w))) - a) / (y - a),
        oe = We.CesiumMath.clamp(oe, 0, 1),
        fe = ne === te,
        ue = ne === ae - 1;
      0 < m && (fe ? (le += 1e-5 * E) : ue && (le -= 1e-5 * E));
      var ce,
        de = r(le),
        he = n(le),
        me = _ * he;
      ...
      for (var ge = ie; ge < re; ++ge) {
        ...
        if (1 === A) ke = c[we];
          ...
          var xe,
            ke = 0;
          if (S) for (xe = 0; xe < A; ++xe) ke = ke * D + c[we + xe];
          else for (xe = A - 1; 0 <= xe; --xe) ke = ke * D + c[we + xe];
        }
        (ke = (ke * M + V) * U), (Y = Math.max(Y, ke)), (H = Math.min(H, ke));
        var ye = x.west + C * pe;
        g ? (ye = u(ye)) : (ye *= w);
        var Ie = (ye - t) / (i - t),
        ...
        if (0 < m) {
          var Ue = ge === ie,
            ve = ge === re - 1,
            Te = fe || ue || Ue || ve;
          if ((fe || ue) && (Ue || ve)) continue;
          Te &&
            ((ke -= m),
            Ue
              ? ((be = h - se - 1 + q), (ye -= 1e-5 * P))
              : ue
              ? (be = q + h + (d - pe - 1))
              : ve
              ? ((be = q + h + d + se), (ye += 1e-5 * P))
              : fe && (be = q + h + d + h + pe));
        }
        var Me = de * r(ye),
          Ve = de * n(ye),
    ...
    Ye.defined(k) && (Fe = qe.OrientedBoundingBox.fromRectangle(k, H, Y, p)),
      b &&
    ...
    for (
      var Ne = new Ge.AxisAlignedBoundingBox(Z, j, I),
        ze = new Qe.TerrainEncoding(Ne, G, Y, W, !1, v),
      ...
      for (var g = 0, p = 0; p < m; p++)
        for (var w = 0; w < h; w++) {
          ...
          var I,
            ...
            U,
            v = n.getUint8(0);
          if ((x++, (y.encoding = 63 & v), 3 < y.encoding))
            throw 'Invalid block encoding (' + y.encoding + ')';
          if (2 !== y.encoding) {
            if (0 !== v && 2 !== v) {
              if (((v >>= 6), 2 === (y.offsetType = v)))
                (y.offset = n.getInt8(1)), x++;
              else if (1 === v) (y.offset = n.getInt16(1, !0)), (x += 2);
              else {
            ...
            if (((t += x), 3 !== y.encoding))
              if (0 === y.encoding) {
                ...
                1 === y.encoding &&
                  ((I = Math.ceil((y.numValidPixels * y.bitsPerPixel) / 8)),
                  (b = Math.ceil(I / 4)),
                  (U = new ArrayBuffer(4 * b)),
                  new Uint8Array(U).set(new Uint8Array(e, t, I)),
    ...
    (R = {
      ...
      readMask: function (e, t) {
        ...
        if (0 === l)
          ...
          a = new Uint8Array(Math.ceil(s / 8));
          var u,
            c = (o = new DataView(e, r, f.numBytes)).getInt16(0, !0),
            d = 2,
            h = 0;
          do {
            if (0 < c) for (; c--; ) a[h++] = o.getUint8(d++);
            else for (u = o.getUint8(d++), c = -c; c--; ) a[h++] = u;
            (c = o.getInt16(d, !0)), (d += 2);
          } while (d < f.numBytes);
          if (-32768 !== c || h < a.length)
            throw 'Unexpected end of mask RLE encoding';
          i = new Uint8Array(s);
          for (var m = 0, g = 0, g = 0; g < s; g++)
      ...
      readDataOneSweep: function (e, t, a) {
        var i,
          ...
          c =
            a === Uint8Array
              ? new Uint8Array(e, r, f)
              : ((i = new ArrayBuffer(f)),
                new Uint8Array(i).set(new Uint8Array(e, r, f)),
                new a(i));
        if (c.length === l * s) t.pixels.resultPixels = c;
        else {
          ...
          if (1 < s)
            for (g = 0; g < s; g++)
              for (d = g * l, m = 0; m < l; m++)
                u[m] && (t.pixels.resultPixels[d + m] = c[h++]);
          else
            for (m = 0; m < l; m++) u[m] && (t.pixels.resultPixels[m] = c[h++]);
        }
      ...
      readHuffmanTree: function (e, t) {
        ...
        for (d = n; d < s; d++)
          0 < (u = c[(o = d - (d < r ? 0 : r))].first) &&
            ...
            u <= 32 - w
              ? 32 === (w += u) && ((w = 0), (k = p[++x]))
              : ((w += u - 32), (k = p[++x]), (c[o].second |= k >>> (32 - w))));
        var y = 0,
          I = 0,
          b = new C();
        for (d = 0; d < c.length; d++)
          void 0 !== c[d] && (y = Math.max(y, c[d].first));
        (I = a <= y ? a : y),
        ...
        for (d = n; d < s; d++)
          if (0 < (u = c[(o = d - (d < r ? 0 : r))].first))
            if (((U = [u, o]), u <= I))
              for (
                v = c[o].second << (I - u), T = 1 << (I - u), f = 0;
                f < T;
                f++
              )
                A[v | f] = U;
            else
              for (v = c[o].second, V = b, M = u - 1; 0 <= M; M--)
                (V =
                  (v >>> M) & 1
      ...
      readHuffman: function (e, t, a) {
        ...
        0 < b && (I++, (b = 0));
        for (
          var A = y[I], B = 1 === t.encodeMode, D = new a(p * h), S = D, P = 0;
          P < d.numDims;
          P++
        ) {
          if (
            (1 < h && ((S = new a(D.buffer, p * P, p)), (V = 0)),
              ...
              for (f = 0; f < g; f++, u++) {
                if (
                  ...
                  for (
                    ...
                    if (
                      !(i = (s >>> (v - c - 1)) & 1 ? i.right : i.left).left &&
                      !i.right
                    ) {
                      (r = i.val), (b = b + c + 1);
                      break;
      ...
      readTiles: function (e, t, a) {
        ...
        for (
          ...
          for (c = A !== u - 1 ? s : F, B = 0; B < f; B++)
            for (
              D = A * r * s + B * s, p = r - (d = B !== f - 1 ? s : L), T = 0;
              T < O;
              T++
            ) {
              if (
              ...
              if (3 < (b = 3 & m))
                throw ((t.ptr += E), 'Invalid block encoding (' + b + ')');
              if (2 != b)
                if (0 == b) {
                  if (
                    (t.counter.uncompressed++,
                    (t.ptr += E),
                    (S = (S = c * d * o) < (w = e.byteLength - t.ptr) ? S : w),
                    (y = new ArrayBuffer(S % o == 0 ? S : S + o - (S % o))),
                    new Uint8Array(y).set(new Uint8Array(e, t.ptr, S)),
                    (I = new a(y)),
                    (P = 0),
                    N)
                  )
                    for (M = 0; M < c; M++) {
                      for (V = 0; V < d; V++) N[D] && (z[D] = I[P++]), D++;
                      D += p;
                    }
                  else
                    for (M = 0; M < c; M++) {
                      for (V = 0; V < d; V++) z[D++] = I[P++];
                      D += p;
                    }
                  t.ptr += P * o;
                } else if (
                  ((U = R.getDataTypeUsed(l, g)),
                  (v = R.getOnePixel(k, E, U, x)),
                  (E += R.getDataTypeSize(U)),
                  3 == b)
                )
                  if (((t.ptr += E), t.counter.constantoffset++, N))
                    for (M = 0; M < c; M++) {
                      for (V = 0; V < d; V++) N[D] && (z[D] = v), D++;
                      D += p;
                    }
                  else
                    for (M = 0; M < c; M++) {
                  ...
                  ((t.ptr += E), R.decodeBits(e, t, C, v, T), (E = 0), N)
                )
                  for (M = 0; M < c; M++) {
                    for (V = 0; V < d; V++) N[D] && (z[D] = C[E++]), D++;
                    D += p;
                  }
                else
                  for (M = 0; M < c; M++) {
                    for (V = 0; V < d; V++) z[D++] = C[E++];
                    D += p;
                  }
      ...
      decode: function (e, t) {
        ...
        if (
          ...
          if (r.pixels.resultMask && R.isValidPixelValue(n.imageType, a)) {
            for (var m = r.pixels.resultMask, i = 0; i < f; i++)
              m[i] || (h.pixelData[i] = a);
            h.noDataValue = a;
          }
          return (
