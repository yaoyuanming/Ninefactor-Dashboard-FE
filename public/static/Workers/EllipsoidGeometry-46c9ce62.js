define([
  ...
  'use strict';
  var ht = new ct.Cartesian3(),
    vt = new ct.Cartesian3(),
    At = new ct.Cartesian3(),
    xt = new ct.Cartesian3(),
    kt = new ct.Cartesian3(),
  ...
  (C.unpack = function (t, e, a) {
    ...
    return ut.defined(a)
      ...
      : ((h.minimumClock = o),
        ...
        (h.stackPartitions = l),
        (h.slicePartitions = c),
        (h.offsetAttribute = -1 === f ? void 0 : f),
        new C(h));
  }),
    (C.createGeometry = function (t) {
      ...
      if (!(e.x <= 0 || e.y <= 0 || e.z <= 0)) {
        ...
        if (!(a.x <= 0 || a.y <= 0 || a.z <= 0)) {
          ...
          (s = Math.round((s * Math.abs(r - i)) / lt.CesiumMath.TWO_PI)) < 2 &&
            (s = 2),
            (u = Math.round((u * Math.abs(o - n)) / lt.CesiumMath.PI)) < 2 &&
              (u = 2);
          var l = 0,
            c = [n],
            f = [i];
          for (W = 0; W < u; W++) c.push(n + (W * (o - n)) / (u - 1));
          for (c.push(o), E = 0; E < s; E++)
            f.push(i + (E * (r - i)) / (s - 1));
          f.push(r);
          var d = c.length,
            C = f.length,
          ...
          _ &&
            ...
            0 < n && ((h = !0), (p += s - 1)),
            o < Math.PI && ((v = !0), (p += s - 1)),
            (r - i) % lt.CesiumMath.TWO_PI
              ? ((A = !0), (p += 2 * (u - 1) + 1))
              : (p += 1));
          var x = C * d * y,
            k = new Float64Array(3 * x),
            b = _t.arrayFill(new Array(x), !1),
            w = _t.arrayFill(new Array(x), !1),
            F = s * u * y,
            P = 6 * (F + p + 1 - (s + u) * y),
            g = yt.IndexDatatype.createTypedArray(F, P),
            V = m.normal ? new Float32Array(3 * x) : void 0,
            M = m.tangent ? new Float32Array(3 * x) : void 0,
            T = m.bitangent ? new Float32Array(3 * x) : void 0,
            D = m.st ? new Float32Array(2 * x) : void 0,
            G = new Array(d),
            L = new Array(d);
          for (W = 0; W < d; W++) (G[W] = wt(c[W])), (L[W] = bt(c[W]));
          for (var O = new Array(C), I = new Array(C), E = 0; E < C; E++)
            (I[E] = bt(f[E])), (O[E] = wt(f[E]));
          for (W = 0; W < d; W++)
            for (E = 0; E < C; E++)
              (k[l++] = e.x * G[W] * I[E]),
                (k[l++] = e.y * G[W] * O[E]),
            ...
            for (W = 0; W < d; W++)
              for (E = 0; E < C; E++)
                (k[l++] = a.x * G[W] * I[E]),
                  ...
                  (k[l++] = a.z * L[W]),
                  (b[S] = !0),
                  0 < W && W !== d - 1 && 0 !== E && E !== C - 1 && (w[S] = !0),
                  S++;
          for (l = 0, W = 1; W < d - 2; W++)
            for (z = W * C, N = (W + 1) * C, E = 1; E < C - 2; E++)
              (g[l++] = N + E),
                (g[l++] = N + E + 1),
                (g[l++] = z + E + 1),
                (g[l++] = N + E),
                ...
                (g[l++] = z + E);
          if (_)
            for (var B = d * C, W = 1; W < d - 2; W++)
              for (z = B + W * C, N = B + (W + 1) * C, E = 1; E < C - 2; E++)
                (g[l++] = N + E),
                  (g[l++] = z + E),
                  (g[l++] = z + E + 1),
                  (g[l++] = N + E),
            ...
            if (h)
              for (U = d * C, W = 1; W < C - 2; W++)
                (g[l++] = W),
                  ...
                  (g[l++] = U + W);
            if (v)
              for (R = d * C - C, U = d * C * y - C, W = 1; W < C - 2; W++)
                (g[l++] = R + W + 1),
                  (g[l++] = R + W),
                  (g[l++] = U + W),
                  (g[l++] = R + W + 1),
          ...
          var J,
            ...
            Q = 0,
            $ = x / 2,
            tt = ct.Ellipsoid.fromCartesian3(e),
            et = ct.Ellipsoid.fromCartesian3(a);
          if (m.st || m.normal || m.tangent || m.bitangent) {
            for (W = 0; W < x; W++) {
              ...
              var at,
                ...
                nt,
                ot,
                mt = ct.Cartesian3.fromArray(k, 3 * W, ht),
                st = J.geodeticSurfaceNormal(mt, vt);
              w[W] && ct.Cartesian3.negate(st, st),
                m.st &&
                  ((at = ct.Cartesian2.negate(st, kt)),
                  (D[q++] =
                    Math.atan2(at.y, at.x) / lt.CesiumMath.TWO_PI + 0.5),
                  (D[q++] = Math.asin(st.z) / Math.PI + 0.5)),
                m.normal && ((V[H++] = st.x), (V[H++] = st.y), (V[H++] = st.z)),
                (m.tangent || m.bitangent) &&
                  ...
                  ct.Cartesian3.cross(nt, st, it),
                  ct.Cartesian3.normalize(it, it),
                  m.tangent &&
                    ((M[K++] = it.x), (M[K++] = it.y), (M[K++] = it.z)),
                  m.bitangent &&
                    ((ot = ct.Cartesian3.cross(st, it, xt)),
                    ct.Cartesian3.normalize(ot, ot),
                    (T[Q++] = ot.x),
                    (T[Q++] = ot.y),
                    (T[Q++] = ot.z)));
            }
          ...
          return (
            ut.defined(t._offsetAttribute) &&
              ((X = k.length),
              (Z = new Uint8Array(X / 3)),
              (j =
            ...
            new Ct.Geometry({
              ...
              boundingSphere: ft.BoundingSphere.fromEllipsoid(tt),
