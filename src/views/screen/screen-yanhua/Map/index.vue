...
<script lang="ts" setup>
  ...
  // 从父组件注入 openDrawer 方法
  const openDrawer =
    inject<(type: any, title: string, data?: any) => void>('openDrawer');
  ...
  function ViewEnterpriseDetails(rows: any) {
    if (openDrawer) {
      // 传递企业 ID，让 DetailDrawer 调用 CompanyDetail 组件的 getRecord 方法
      openDrawer(DrawerType.COMPANY, '企业概况', { id: rows.id });
    }
  }
    ...
    try {
      ...
      const loadAllDataIncremental = async (pageNo = 1) => {
        const res = await getEnterPageList({
          ...
          enterpriseName: searchKeyword.value, // 添加搜索参数
        });
