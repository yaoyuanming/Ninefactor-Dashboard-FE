<template>
  <div class="company-detail-container">
    ...
    <div class="left-column">
        ...
        <div class="core-info-wrap">
          ...
          <div class="company-base-info">
            <div class="company-name">{{
              basicInfo.find((item) => item.label === '企业名称').value
            }}</div>
            <div class="credit-code"
              >统一社会信用代码：{{
                basicInfo.find((item) => item.label === '统一社会信用代码')
                  .value
              }}</div
            >
          </div>
      ...
      <div class="detail-section">
        <div class="section-header">
          <h3 class="section-title">高危环境与经营信息</h3>
        </div>
        <div class="basic-info-list">
          <div
            v-for="item in riskAndBusinessInfo"
            :key="item.label"
            class="basic-info-item"
          >
            <span class="item-label">{{ item.label }}：</span>
          ...
          <div v-for="(item, idx) in attachmentInfo" :key="idx">
            ...
            <div class="attachment-list">
              <template v-if="item.value.length > 0">
                <div
                  v-for="(img, imgIdx) in item.value"
                  :key="imgIdx"
        ...
        <a-tabs v-model:active-key="activeTabKey" size="large">
          ...
          <a-tab-pane key="warehouse" title="仓库">
            <a-table
              ...
              class="table"
              @change="
                (pagination) => handleTableChange(pagination, 'warehouse')
              "
            />
          </a-tab-pane>
          <!-- 库房表格（带分页） -->
          <a-tab-pane key="storehouse" title="库房">
            <a-table
              :columns="storehouseColumns"
              :data="storehouseData"
              bordered
              :pagination="storehousePagination"
              :scroll="{ x: 'max-content' }"
              class="table"
              @change="
                (pagination) => handleTableChange(pagination, 'storehouse')
              "
            />
          </a-tab-pane>
          ...
          <a-tab-pane key="personnel" title="人员">
            <a-table
              ...
              :scroll="{ x: 'max-content' }"
              @change="
                (pagination) => handleTableChange(pagination, 'personnel')
              "
            >
              <template #avatar="{ record }">
                <a-image
                  ...
                  fit="cover"
                  class="avatar-img"
                  @click="record.avatar && viewImage(record.avatar)"
                />
              </template>
            </a-table>
          </a-tab-pane>
          ...
          <a-tab-pane key="vehicle" title="车辆">
            <a-table
              ...
              class="table"
              :scroll="{ x: 'max-content' }"
              @change="(pagination) => handleTableChange(pagination, 'vehicle')"
            >
...
<script setup>
  ...
  const basicInfo = computed(() => {
    ...
    return [
      ...
      {
        label: '企业规模',
        value: enterpriseScaleMap[data.enterpriseScale] || '---',
      },
      { label: '职工总人数', value: data.employeeCount || '---' },
      {
        label: '最近填报时间',
        value: data.lastReportTime ? formatTime(data.lastReportTime) : '---',
      },
      {
        label: '经度',
        value: data.longitude ? data.longitude.toFixed(6) : '---',
      },
      {
        label: '纬度',
        value: data.latitude ? data.latitude.toFixed(6) : '---',
      },
      {
        label: '创建时间',
      ...
      {
        label: '更新时间',
        value: data.updateTime ? formatTime(data.updateTime) : '---',
      },
    ];
  });
    ...
    return [
      {
        label: '上一年营业收入',
        value:
          data.lastYearRevenue !== undefined
            ? `${data.lastYearRevenue} 万元`
            : '---',
      },
      {
        label: '企业注册地址',
    ...
    return [
      {
        ...
        value: data.riskEnvTypes
          ? data.riskEnvTypes
              .split(',')
              .map((code) => riskEnvMap[code] || `未知类型(${code})`)
              .join('、')
          : '---',
      },
      {
  ...
  const attachmentInfo = computed(() => {
    ...
    return [
      {
        label: '许可证附件',
        value: data.licenseAnnex
          ? data.licenseAnnex.split(',').filter((img) => img.trim())
          : [],
      },
    ];
  });
  ...
  const vehicleColumns = ref([
    ...
    {
      title: '运营许可号',
      dataIndex: 'operationalLicenseNumber',
      key: 'operationalLicenseNumber',
      align: 'center',
    },
    {
      title: '车辆证件',
      slotName: 'vehicleDocumentUrls',
      key: 'vehicleDocumentUrls',
      align: 'center',
    },
    {
      title: '更新日期',
  ...
  ::v-deep(.arco-page-header) {
    ...
    .arco-page-header-back {
      ...
      &:hover {
        ...
        background: rgb(23 150 250 / 15%) !important;
      }
  ...
  .core-info {
    .core-info-wrap {
      ...
      .avatar {
        ...
        height: 32px;
        object-fit: cover;
        border: 2px solid rgb(23 150 250 / 40%);
        border-radius: 8px;
      }
      ...
      .company-name {
        ...
        font-weight: 600;
        font-size: 16px;
        text-shadow: 0 0 8px rgb(23 150 250 / 30%);
      }
  ...
  .detail-section {
    ...
    .section-header {
      ...
      .section-title {
        ...
        font-weight: 600;
        font-size: 16px;
        text-shadow: 0 0 8px rgb(23 150 250 / 40%);
    ...
    .attachment-list {
      ...
      .attachment-item {
        ...
        &:hover {
          ...
          border-color: rgb(23 150 250 / 40%);
          box-shadow: 0 4px 12px rgb(23 150 250 / 20%);
    ...
    ::v-deep(.arco-table) {
      ...
      * {
      ...
      .arco-empty {
        ...
        .arco-empty-image {
          filter: brightness(0.7) sepia(1) hue-rotate(200deg) saturate(0.8);
        }
