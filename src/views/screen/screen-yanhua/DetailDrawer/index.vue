<template>
  <a-drawer :visible="visible" :title="title" :width="1200" :footer="false" :mask-closable="true" :closable="true"
    placement="bottom" :height="'70vh'" unmount-on-close class="detail-drawer" @cancel="handleClose">
    <div class="drawer-content">
      <CompanyTables ref="CompanyTable" v-if="type === DrawerType.COMPANY" :data="data" />
      <WarehouseTable v-if="type === DrawerType.WAREHOUSE" :data="data" />
      <CreateRegionalTask v-if="type === DrawerType.REGIONAL_TASK" @close="handleClose" @success="handleSuccess" />
      <CreateEnterpriseTask v-if="type === DrawerType.ENTERPRISE_TASK" @close="handleClose" @success="handleSuccess" />
      <ViewRegionalTask v-if="type === DrawerType.VIEW_REGIONAL_TASK" :task-id="data?.id || data" />
      <ViewEnterpriseTask v-if="type === DrawerType.VIEW_ENTERPRISE_TASK" :task-id="data?.id || data" />
