      ...
      <div class="create-content">
        ...
        <div class="form-section">
          ...
          >
            ...
            <a-form-item label="所属单位" field="orgId" required>
              <DeptSelect
...
<script lang="ts" setup>
  ...
  const formRules = {
    ...
    orgId: [{ required: true, message: '请选择所属单位' }],
  };

  // 处理部门选择变化
  const handleDeptChange = (value: number, deptNames?: string) => {
    formData.orgId = value;
    formData.orgName = deptNames || '';
  };
  ...
  const loadDetail = async () => {
    if (props.data?.id) {
      ...
      try {
        ...
        formData.planType = detail.planType || 1;
        formData.orgType = detail.orgType || 1;
