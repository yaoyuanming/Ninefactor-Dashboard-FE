      ...
      <div class="create-content">
        ...
        <div class="form-section">
          ...
          >
            <a-row :gutter="16">
              <a-col :span="12">
                <a-form-item label="物资名称" field="suppliesName" required>
                  <a-input
            ...
            <a-row :gutter="16">
              <a-col :span="12">
                <a-form-item label="物资类型" field="suppliesType" required>
                  <a-select
                    v-model="formData.suppliesType"
                    placeholder="请选择物资类型"
                  >
                    <a-option :value="1">防护用品</a-option>
                    <a-option :value="2">生命救助</a-option>
                    <a-option :value="3">生命支持</a-option>
                    <a-option :value="4">救援运载</a-option>
                    <a-option :value="5">临时食宿</a-option>
        ...
        <div class="upload-section">
          <div class="section-title">物资图片</div>
          <ImageUpload
...
<script lang="ts" setup>
  ...
  const props = defineProps<{ editData?: EmergencySuppliesVO }>();
  ...
  const formData = reactive<EmergencySuppliesVO & { tempImageUrls?: string }>({
    suppliesName: '',
    warehouseName: '',
    suppliesType: 1,
    quantity: undefined,
  ...
  // 表单验证规则
  const rules = {
    suppliesName: [{ required: true, message: '请输入物资名称' }],
    warehouseName: [{ required: true, message: '请输入仓库名称' }],
    suppliesType: [{ required: true, message: '请选择物资类型' }],
  ...
  const loadDetail = async () => {
    ...
    loading.value = true;
    try {
      const response: any = await getEmergencySupplies(props.editData.id);
      const detail = response.data;

      // 回显数据
      formData.suppliesName = detail.suppliesName || '';
      formData.warehouseName = detail.warehouseName || '';
      formData.suppliesType = detail.suppliesType || 1;
      ...
      Message.error(error?.message || '加载物资详情失败');
    } finally {
      loading.value = false;
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitData: EmergencySuppliesVO = {
        suppliesName: formData.suppliesName,
        warehouseName: formData.warehouseName,
        suppliesType: formData.suppliesType,
      ...
      if (props.editData?.id) {
        await updateEmergencySupplies({ ...submitData, id: props.editData.id });
        Message.success('更新成功');
      } else {
        await createEmergencySupplies(submitData);
