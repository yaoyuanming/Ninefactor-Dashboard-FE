    ...
    <div class="table-container">
      ...
      >
        <template #columns>
          ...
          <a-table-column title="装备类型" :width="120" align="center">
...
<script lang="ts" setup>
  ...
  import {
    ...
    type EmergencyEquipmentListReqVO,
  } from '@/api/emergency';

  const emit = defineEmits<{
    (e: 'view', data: EmergencyEquipmentVO): void;
    (e: 'create', data?: EmergencyEquipmentVO): void;
  }>();

  const loading = ref(false);
  const tableData = ref<EmergencyEquipmentVO[]>([]);

  // 搜索表单
  const searchForm = reactive<EmergencyEquipmentListReqVO>({
    equipmentName: '',
    warehouseName: '',
  ...
  const fetchData = async () => {
    loading.value = true;
    try {
      const response: any = await getEmergencyEquipmentList({
      ...
      Message.error(error?.message || '获取装备列表失败');
    } finally {
      loading.value = false;
    }
  };
  ...
  const handleReset = () => {
    ...
    searchForm.equipmentType = undefined;
    pagination.current = 1;
    fetchData();
  };
  ...
  // 查看详情
  const handleView = (record: EmergencyEquipmentVO) => {
    emit('view', record);
  };

  // 编辑装备
  const handleEdit = (record: EmergencyEquipmentVO) => {
    emit('create', record);
  };

  // 删除装备
  const handleDelete = (record: EmergencyEquipmentVO) => {
    Modal.confirm({
      title: '删除确认',
      content: `确认删除装备「${record.equipmentName}」吗？`,
      okText: '确认',
      cancelText: '取消',
      onOk: async () => {
        if (!record.id) return;
        try {
          await deleteEmergencyEquipment([record.id]);
