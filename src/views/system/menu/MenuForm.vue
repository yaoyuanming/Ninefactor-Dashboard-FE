...
const formType = ref('') // 表单的类型：create - 新增；update - 修改；copy - 复制
const formData = ref({
  id: undefined,
  name: '',
  permission: '',
...
/** 打开弹窗 */
// const open = async (type: string, id?: number, parentId?: number, menuTreeValue?: any) => {
const open = async (type: string, id?: number, parentId?: number) => {
  ...
  // 获得菜单列表
  await getTree()
  // console.log('menuTreeValue2', menuTreeValue)
  // menuTree.value = menuTreeValue;
}
  ...
  try {
    if (
      ...
      if (!isExternal(formData.value.path)) {
        // let parentIdIsCategory = false
        // getIntDictOptions(DICT_TYPE.SYSTEM_MENU_CATEGORY).forEach((item) => {
        //   if (formData.value.parentId === item.value) {
        //     parentIdIsCategory = true
        //     break
        //   }
        // })
        let parentIdIsCategory = getParentIdIsCategory()
        let isRouteId = formData.value.parentId === 0 || parentIdIsCategory
        if (isRouteId && formData.value.path.charAt(0) !== '/') {
          message.error('根菜单路径必须以 / 开头')
          return
        } else if (!isRouteId && formData.value.path.charAt(0) === '/') {
          message.error('非根菜单路径不能以 / 开头')
          return
        }
      }
    }
...
const getTree = async () => {
  const res = await MenuApi.getSimpleMenusList()
  menuTree.value = handleTree(res)
  // 默认将第一个菜单作为默认的[上级菜单]
  if (menuTree.value.length > 0 && formData.value.parentId === 0) {
    formData.value.parentId = menuTree.value[0].id
  }
}
