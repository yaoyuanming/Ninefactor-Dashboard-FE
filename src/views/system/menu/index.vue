...
let miniValidMenuId = 8 // 最小有效菜单ID
...
const columns = [
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }) => {
      // 检查权限
      if (!checkPermi(['system:menu:update']) || rowData.id < miniValidMenuId) {
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }) => {
      ...
      if (checkPermi(['system:menu:create']) && rowData.id >= miniValidMenuId) {
        buttons.push(
          <ElButton
            key="create"
            link
      ...
      if (checkPermi(['system:menu:copy']) && rowData.id >= miniValidMenuId) {
        buttons.push(
          <ElButton key="copy" link type="primary" onClick={() => openForm('copy', rowData.id)}>
            复制
          </ElButton>
        )
      }
      if (checkPermi(['system:menu:delete']) && rowData.id >= miniValidMenuId) {
        buttons.push(
...
const openForm = (type: string, id?: number, parentId?: number) => {
  // console.log('menuTreeValue1', list.value)
  // formRef.value.open(type, id, parentId, list.value)
