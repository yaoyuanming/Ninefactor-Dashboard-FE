<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="50%">
    ...
    >
      ...
      <el-form-item label="所属平台" prop="description">
        <el-select v-model="formData.platformIdsBack" multiple filterable>
          <el-option
            v-for="(dict, key) in PlatList.list"
            :key="key"
            :value="dict.id"
            :label="dict.name"
          />
      ...
      <el-form-item label="用户密码" :prop="formData.id ==null?'password':''">
...
let PlatList = ref({list:[]})
const getPlatList =async () => {
  PlatList.value = await PlatformApi.getPlatformPage({
    pageNo: 1,
    pageSize: 100,
    isPage: 0
  })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      ...
      if(formData.value.platformIds != null){
        let platformIdsBack = []
        formData.value.platformIds.split(',').forEach(item => {
          platformIdsBack.push(Number(item))
        })
        formData.value.platformIdsBack = platformIdsBack
      }
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    data.platformIds = data.platformIdsBack?.join(',')
