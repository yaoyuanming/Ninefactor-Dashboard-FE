...
import {
  ...
  OperationButtonType,
  CandidateStrategy
} from '@/components/SimpleProcessDesignerV2Tenant/src/consts'
...
const getButtonState  = (btnType) => {
  let state = false
  runningTask.value?.buttonsSetting.forEach(i=>{
    if(btnType == i.id){
      state = i.enable
    }
  })
...
const isShowButton = (btnType: OperationButtonType): boolean => {
  let isShow = true
  if (runningTask.value?.buttonsSetting ) {
    isShow = getButtonState(btnType)
  }
  return isShow
}
...
const getButtonDisplayName = (btnType: OperationButtonType) => {
  ...
  if (runningTask.value?.buttonsSetting ) {
    displayName = getButtonName(btnType)
  }
  return displayName
}
