<template>
  <ContentWrap>
    <div class="flex justify-between pl-20px items-center">
      <h3 class="font-extrabold">流程管控</h3>
      <!-- 搜索工作栏 -->
      <el-form
      ...
      >
        ...
        <!-- 右上角：新建模型、更多操作 -->
        <el-form-item>
          <el-button type="primary" @click="openForm('create')" v-hasPermi="['bpm:model-tenant:create']">
...
const openForm = (type: string, id?: number) => {
  if (type === 'create') {
    // push({ name: 'BpmModelTenantCreate' })
    // 打开弹窗
...
const getList = async () => {
  ...
  try {
    ...
    categoryGroup.value = categoryList.map((category: any) => ({
      ...category,
      modelList: modelList.filter((model: any) => {
        return model.category == category.code;
      })
    }));
  } finally {
    loading.value = false
