    ...
    <div class="mx-auto">
      ...
      >
        ...
        <div class="w-200px flex items-center justify-end gap-2">
          ...
          <el-button type="primary" @click="handleSave" v-loading="loading" :disabled="loading">
      ...
      <!-- 主体内容 -->
      <div class="mt-50px">
<!--        <p style="color: red;">企业端</p>-->
        <!-- 第一步：基本信息 -->
        <div v-if="currentStepName === '基本信息'" class="mx-auto w-560px">
          <BasicInfo
            v-model="formData"
        ...
        <!-- 第二步：表单设计 -->
        <div v-if="currentStepName === '表单设计'" class="mx-auto w-560px">
          <FormDesign v-model="formData" :formList="formList" ref="formDesignRef" />
        </div>

        <!-- 第三步：流程设计 -->
        <ProcessDesign  v-if="currentStepName === '流程设计'" v-model="formData" ref="processDesignRef" />

        <!-- 第四步：更多设置 -->
        <div  v-show="currentStepName === '更多设置'" class="mx-auto w-700px">
...
import * as ModelApi from '@/api/bpm/modelTenant'
import * as FormApi from '@/api/bpm/form'
import { CategoryApi, CategoryVO } from '@/api/bpm/category'
import * as UserApi from '@/api/system/user'
import * as DefinitionApi from '@/api/bpm/definitionTenant'
...
const currentStepName = ref("基本信息")
const steps = ref([
...
const initData = async () => {
  if (actionType === 'definition') {
  ...
  } else if (['update', 'copy', 'test'].includes(actionType)) {
    // 情况二：修改场景/复制场景
    ...
    // 企业端新建
    if(actionType === 'test'){
      formData.value.baseModelId = formData.value.id
      formData.value.key = formData.value.key + '_1'
      delete formData.value.id
      tagsView.setTitle('新建流程')
      console.log("formData.value.baseModelId",formData.value.baseModelId)
    }
    if(actionType === 'update'){
      let newItem = await ModelApi.getModel(formData.value.baseModelId)
      console.log("-------newItem.baseDefinitionVersion--------------",newItem.baseDefinitionVersion)
      console.log("-------formData.value.baseDefinitionVersion--------------",formData.value.baseDefinitionVersion)
      if(newItem.baseDefinitionVersion > formData.value.baseDefinitionVersion){
        console.log("---------------------")
      }
      // 请求创建时的数据,如果旧的数据的版本发生了变化，则提示用户是否采用新的数据
      // formData.value = await ModelApi.getModel(formData.value.baseModelId)
    }
  ...
  // 如果formData.value.allowUpdateExtraSetting为false的话，从steps中移除title为更多设置的步骤
  if (formData.value.allowUpdateExtraSetting === false) {
    const index = steps.value.findIndex(step => step.title === "更多设置");
    if (index !== -1) {
      steps.value.splice(index, 1);
    }
  }
  currentStepName.value = steps.value[0].title
  // 获取表单列表
...
const handleSave = async () => {
  try {
    ...
    // 返回列表页（排除更新的情况）
    if (actionType !== 'update') {
      await router.push({ name: 'BpmModelTenant' })
      loading.value = false
    }
  } catch (error: any) {
...
/** 步骤切换处理 */
const handleStepClick = async (index: number,item:any) => {
  currentStepName.value = item.title
  try {
    if (index !== 0) {
      await validateBasic()
    }
...
const handleBack = () => {
  ...
  // 跳转到列表页
  router.push({ name: 'BpmModelTenant' })
}
