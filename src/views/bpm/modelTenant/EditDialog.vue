<template>
  <Dialog v-model="dialogVisible" width="1000px" title="模型选择">
    <!--    需要修改左侧点击切换菜单-->
              ...
              >
                ...
                <div class="grid grid-cols-3 gap3">
                  <el-tooltip
                    ...
                    :disabled="
                      !definition.description || definition.description.trim().length === 0
                    "
                    placement="top"
                  >
                    <el-card
                      ...
                      @click="handleSelect(definition, 'test')"
                    >
                      <template #default>
                        <div class="flex">
                          <el-image
...
import * as Definition from '@/api/bpm/processDefinition'
// 调用接口数据
// const getList = async () => {
//   loading.value = true
//   try{
//     list.value = await Definition.getDefinitionList({suspensionState:1})
//
//   }finally {
//     loading.value = false
//   }
...
const processDefinitionGroup: any = computed(() => {
  ...
  const orderedGroup = {}
  categoryList.value.forEach((category: any, index: number) => {
...
/** 处理选择流程的按钮操作 **/
const handleSelect = async (row, type) => {
  let id = row.modelId
  // 设置选择的流程
