...
const hasPermiUpdate = computed(() => {
  return checkPermi(['bpm:model-tenant:update'])
})
const hasPermiDelete = computed(() => {
  return checkPermi(['bpm:model-tenant:delete'])
})
const hasPermiDeploy = computed(() => {
  return checkPermi(['bpm:model-tenant:deploy'])
})
const hasPermiMore = computed(() => {
  return checkPermi(['bpm:process-definition-tenant:query', 'bpm:model-tenant:update', 'bpm:model-tenant:delete'])
})
const hasPermiPdQuery = computed(() => {
  return checkPermi(['bpm:process-definition-tenant:query'])
})
...
const openModelForm = async (type: string, id?: number) => {
  if (type === 'create') {
    await push({ name: 'BpmModelTenantCreate' })
  } else {
    await push({
