<template>
  ...
  >
    ...
    <el-tabs type="border-card" v-model="activeTabName" v-if="approveType === ApproveType.USER">
      ...
      <el-tab-pane
        label="操作按钮设置"
        v-if="currentNode.type === NodeType.USER_TASK_NODE && modelItem?.allowUpdateNodeListener"
        name="buttons"
      >
      ...
      <el-tab-pane label="监听器" name="listener" v-if="modelItem?.allowUpdateNodeListener">
        <UserTaskListenerTenantTenant
...
import UserTaskListenerTenantTenant from './components/UserTaskListenerTenant.vue'
defineOptions({
  name: 'UserTaskNodeConfigTenant'
})
...
const saveConfig = async () => {
  ...
  let valid = null
  if(props.modelItem?.allowUpdateNodeListener){
     valid = (await formRef.value.validate()) && (await userTaskListenerRef.value.validate())
  }
