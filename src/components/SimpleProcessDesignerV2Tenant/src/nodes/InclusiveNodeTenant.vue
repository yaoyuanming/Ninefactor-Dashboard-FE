  ...
  <div class="branch-node-wrapper">
    <div class="branch-node-container">
      ...
      <el-button  v-else class="branch-node-add" color="#345da2" @click="addCondition" plain
        ><span v-if="modelItem?.allowUpdateProcess">添加条件</span></el-button
      >
      <div
      ...
      >
        ...
        <div class="node-wrapper">
          <div class="node-container">
            ...
            >
              ...
              >
                <div class="toolbar-icon">
                  <Icon
                    v-if="modelItem?.allowUpdateProcess"
                    color="#0089ff"
            ...
            <NodeHandlerTenant v-model:child-node="item.childNode" :current-node="item" />
          </div>
        </div>
        <ConditionNodeConfigTenant :node-index="index" :condition-node="item" :ref="item.id" />
        <!-- 递归显示子节点  -->
...
import NodeHandlerTenant from '../NodeHandlerTenant.vue'
import ProcessNodeTreeTenant from '../ProcessNodeTreeTenant.vue'
...
import ConditionNodeConfigTenant from '../nodes-config/ConditionNodeConfigTenant.vue'
const { proxy } = getCurrentInstance() as any
defineOptions({
  name: 'InclusiveNodeTenant'
})
...
const addCondition = () => {
  // 为假返回
  if(!props.modelItem?.allowUpdateProcess){
    return
  }
  console.log("包容新增，如果是企业则不能新增")
