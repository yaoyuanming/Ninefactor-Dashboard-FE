<template>
  ...
  >
    ...
    <el-tabs type="border-card" v-model="activeTabName" v-if="approveType === ApproveType.USER">
      ...
      <el-tab-pane label="表单设置" name="formSettings" v-if="flowNode.formSetting != null">
        <FormStrings
          ref="formStringsRef"
          :flow-node="flowNode.formSetting"
...
import FormStrings from './components/FormStrings.vue'
defineOptions({
...
const saveConfig = async () => {
  ...
  // 添加表单设置流程校验
  if (!formStringsRef) return false
  if(props.flowNode.formSetting != null){
    const formvalid = await formStringsRef.value.validate()
