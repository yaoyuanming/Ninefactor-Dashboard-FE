...
export type FormSetting = {
  taskFormType?:number  //表单类型,示例值(10)
  formId?:number  //表单类型,示例值(10)
  taskFormPathType?:number  //表单类型,示例值(10)
  taskFormCustomPath?:string  //自定义表单的企业端路径，使用 Vue 的路由地址,示例值(/bpm/oa/leave/approve)
  taskFormCustomAppPath?:string  //自定义表单的移动端路径，使用 Vue 的路由地址,示例值(/bpm/oa/leave/approve)
}
...
export function useNodeName2(node: Ref<SimpleFlowNode>, nodeType: NodeType) {
  console.log("node------------",node.value.formSetting)
  // 显示节点名称输入框
  const showInput = ref(false)
  // 节点名称输入框失去焦点
  const blurEvent = () => {
