...
const visitedViews = computed(() => 
  tagsViewStore.getVisitedViews.filter(view => !view.meta?.hideTopMenu)
)
...
const addTags = () => {
  ...
  console.log("-----新增tag-----addTags---------",name)
  if (name) {
...
const closeSelectedTag = (view: RouteLocationNormalizedLoaded) => {
  closeCurrent(view, () => {
    if (isActive(view)) {
      ...
      if (latestView) {
        // 获取即将跳转的视图的一级路由
        const nextRoute = latestView.path.split('/')[1]
        // 遍历路由找到对应的一级菜单
...
const closeAllTags = () => {
  ...
  if (visitedViews.length > 0) {
    ...
    // 重新添加第一个标签页
    tagsViewStore.addVisitedView(firstView)
    // 设置选中第一个标签页
    setSelectTag(firstView)
    // 跳转到第一个标签页
    push(firstView)
  }
}
            ...
            <div>
              <router-link :ref="tagLinksRefs.set" v-slot="{ navigate }" :to="{ ...item }" custom>
                <div
                  ...
                  @click="() => {
                    // 获取当前点击标签页的路径的第一段作为一级菜单
                    const nextRoute = item.path.split('/')[1]
                    // 遍历路由找到对应的一级菜单
                    permissionStore.getRouters.forEach((route) => {
                      if (route.path === `/${nextRoute}`) {
                        // 设置左侧菜单路由前缀
