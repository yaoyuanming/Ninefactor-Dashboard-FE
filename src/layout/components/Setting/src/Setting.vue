...
// 主题色相关
const systemTheme = computed(() => appStore.getTheme.elColorPrimary)
console.log("1111111111111111111111111",systemTheme)
// systemTheme = ref(appStore.getTheme.elColorPrimary)
...
// 头部主题相关
const headerTheme = computed(() => appStore.getTheme.topHeaderBgColor)
...
const setHeaderTheme = (color: string) => {
  ...
  // 头部菜单
  const topMenuBgActiveColor = isDarkColor ? 'var(--el-color-primary)' : hexToRGB(unref(primaryColor), 0.1)
  ...
  appStore.setTheme({
    ...
    topMenuBgActiveColor: isDarkColor? 'var(--el-color-primary)' : hexToRGB(unref(primaryColor), 0.1), // 头部菜单选中背景颜色
...
// 菜单主题相关 computed(() => appStore.getTheme.leftMenuBgColor)
const menuTheme = computed(() => appStore.getTheme.leftMenuBgColor)
  ...
  const { copy, copied, isSupported } = useClipboard({
    source: `
      ...
      theme: {
        ...
        // 头部字体颜色
        topHeaderSubTextColor: '${appStore.getTheme.topHeaderSubTextColor}',
        // 头部悬停颜色
        topHeaderHoverColor: '${appStore.getTheme.topHeaderHoverColor}',
        // 头部边框颜色
        topToolBorderColor: '${appStore.getTheme.topToolBorderColor}',
        // 头部菜单背景颜色
        topMenuBgColor: '${appStore.getTheme.topMenuBgColor}',
        // 头部菜单选中背景颜色
        topMenuBgActiveColor: '${appStore.getTheme.topMenuBgActiveColor}',
        // 头部菜单字体颜色
        topMenuTextColor: '${appStore.getTheme.topMenuTextColor}',
        // 头部菜单选中字体颜色
        topMenuTextActiveColor: '${appStore.getTheme.topMenuTextActiveColor}'
      }
    `
  })
  if (!isSupported) {
...
<template>
  ...
  <div
    ...
    @click="drawer = true"
  >
    <Icon color="#000" icon="ep:setting" />
  </div>
