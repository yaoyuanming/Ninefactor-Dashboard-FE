...
const water = ref(appStore.getTheme.water)
...
const screenfullChange = (show: boolean) => {
  ...
  appStore.setTheme({ screenFull: show })
}
...
const tagsViewChange = (show: boolean) => {
  ...
  appStore.setTheme({ tagsView: show })
}
...
const tagsViewIconChange = (show: boolean) => {
  ...
  appStore.setTheme({ tagsViewIcon: show })
}
...
const logoChange = (show: boolean) => {
  ...
  appStore.setTheme({ logoChange: show })
}
...
const fixedHeaderChange = (show: boolean) => {
  ...
  appStore.setTheme({ fixedHeader: show })
}
...
const greyModeChange = (show: boolean) => {
  ...
  appStore.setTheme({ greyMode: show })
}
...
const fixedMenuChange = (show: boolean) => {
  ...
  appStore.setTheme({ fixedMenu: show })
}

// 设置水印
const setWater = () => {
  setWatermark(water.value)
  appStore.setTheme({ water: water.value })
}
...
<!-- 
    ...
    <div class="flex items-center justify-between">
      <div class="text-14px w-22">{{ t('watermark.watermark') }}</div>
      <ElInput v-model="water" class="right-1" @change="setWater()" />
    </div>
  </div>
</template>
